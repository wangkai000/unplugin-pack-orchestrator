...
fs.writeFileSync(path.join(projectDir, 'vite.config.mjs'), `
import { defineConfig } from 'vite';
import packPlugin from '${pluginDir.replace(/\\/g, '/').replace(/'/g, "\\'")}/dist/index.js';
...
try {
  await build({
    root: projectDir,
    configFile: path.join(projectDir, 'vite.config.mjs'),
    logLevel: 'info',
  });
